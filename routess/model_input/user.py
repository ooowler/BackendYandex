...
class ShopUnitImport(BaseModel):
    id: str = Field(description='Уникальный идентфикатор', nullable=False,
    ...
    price: Union[int, None] = Field(description='Целое число, для категорий поле должно содержать null.', nullable=True,
                                    example=234)


class ShopUnitImportRequest(BaseModel):
    items: List[ShopUnitImport] = Field(description='Импортируемые элементы', nullable=False)
    updateDate: datetime.datetime = Field(description='Время обновления добавляемых товаров/категорий.', nullable=False,
                                          example="2022-05-28T21:12:01.000Z", default=datetime.datetime.now())
